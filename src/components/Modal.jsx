    ...
    <section className={`fixed top-0 left-0 right-0 bottom-0 grid place-content-center bg-black/70 duration-200 ${isShowModal ? "opacity-100 visible" : "opacity-0 invisible"}`}>
      <form onSubmit={handleSubmit(submit)} className="grid gap-4 w-[300px] relative py-6">
        ...
        <div className="grid gap-1">
          ...
          <textarea {...register("description")} className="text-black rounded-md p-2 outline-none" rows={6} type="text" />
        </div>
        <i onClick={handleClickCloseModal} className='bx bx-x text-3xl absolute right-0 top-0 hover:text-indigo-500 transition-colors cursor-pointer'></i>
        <button className="bg-indigo-500 px-2 rounded-md hover:bg-indigo-400 hover:tracking-wider duration-200 py-2">{taskToEdit ? "Guardar cambios" : "Crear tarea"}</button>
      </form>
    </section>
  )
}
